...
class file_info_container extends file_info {
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (
                ($child = $this->browser->get_file_info($this->context, 'mod_plenum', $this->filearea, $record->itemid))
    ...
    public static function get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
        ...
        if (
            ...
            && !$motion = motion::get_record(['id' => $itemid], IGNORE_MISSING)
        ) {
            return null;
        }
        if (
            $filearea == 'speaker'
            && (
                !($peer = $DB->get_record('plenumform_deft_peer', ['id' => $itemid]))
                || !$motion = motion::get_record(['id' => $peer->motion], IGNORE_MISSING)
            )
        ) {
            return null;
        }
        ...
        if (!($storedfile = $fs->get_file($context->id, 'mod_plenum', $filearea, $itemid, $filepath, $filename))) {
            return null;
        }
        ...
        $fullname = fullname(\core_user::get_user($storedfile->get_userid()));
        ...
        return new file_info_stored(
            ...
            $storedfile,
            $urlbase,
            get_string('pluginname', 'plenumtype_' . $motion->get('type')) . " ($fullname) ",
            true,
            true,
            false,
            false
