...
/**
 * Manage Plenary meeting motion plugins
 *
 * @package    mod_plenum
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class manage_plenumtype_plugins_page extends \admin_setting {
    /**
    ...
    public function __construct() {
        ...
        parent::__construct(
            'manageplenumtype',
            new \lang_string('manageplenumtypeplugins', 'mod_plenum'),
            '',
            ''
        );
    }
    ...
    public function is_related($query): bool {
        ...
        $types = \core_plugin_manager::instance()->get_plugins_of_type('plenumtype');
    ...
    public function output_html($data, $query = ''): string {
        ...
        $types = $pluginmanager->get_plugins_of_type('plenumtype');
        if (empty($types)) {
        ...
        foreach ($types as $type) {
            ...
            $class = '';
            if (
                $pluginmanager->get_plugin_info('plenumtype_' . $type->name)->get_status() ===
